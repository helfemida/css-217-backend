...
import com.sdu.foodorderingsystem.authrepository.LogInModelDAO;
...
public class LogInModelServiceImpl implements LogInModelService {
    ...
    public String LogIn(LogInModel loginData) throws AuthorizationException {
        ...
        Optional<SignUpModel> opt = signUpDAO.findByEmail(loginData.getEmail());
...
//        Optional<SignUpModel> opt = signUpDAO.findById(loginData.getUserId());
        ...
        loginData.setUserId(opt.get().getUserId());
        ...
        if ((newSignUp.getUserId().equals(loginData.getUserId()) && (newSignUp.getPassword().equals(loginData.getPassword())))) {
